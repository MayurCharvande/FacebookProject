...
public class ChangePasswordSyncTask extends AsyncTask<String, Integer, String> {
	
	private ProgressDialog _ProgressDialog;
	Activity mActivity = null;
	ServiceHandler service = null;
	public ChangePasswordSyncTask(Activity activity) {
	...
	protected String doInBackground(String... param) {
		...
		params.add(new BasicNameValuePair("old_pass", Common.USER_OLD_PASSWORD));
		params.add(new BasicNameValuePair("new_pass", Common.USER_NEW_PASSWORD));
		params.add(new BasicNameValuePair("device_type",Common.DEVICE_TYPE));
		
		String strResponce = service.makeServiceCall(WebUrls.USER_CHANGE_PASSWORD_URL, Common.POST, params);
		LogConfig.logd("change password strResponce =",""+strResponce);
		return strResponce;
	}
		...
		if(strResponce != null && strResponce.length() > 0) {
			...
			try {
			   ...
			   if(status.equals("success")) {
				  DisplayAlertSingle(mActivity,"Message", "Password changed successfully");   
			   } else {
