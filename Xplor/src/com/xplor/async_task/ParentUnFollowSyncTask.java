...
public class ParentUnFollowSyncTask extends AsyncTask<String, Integer, String> {
	
	private ProgressDialog _ProgressDialog = null;
	private ServiceHandler service = null;
	private Activity mActivity = null;
	private int position;
	private ParentUnfollowCallBack mParentUnfollowCallBack=null;
	...
	public ParentUnFollowSyncTask(Activity activity,int mposition) {
	...
	protected String doInBackground(String... param) {
		...
		String strResponce = service.makeServiceCall(WebUrls.LEADER_BOARD_PARENT_UNFOLLOW_URL, Common.POST, params);
		return strResponce;
	}
	...
	protected void onPostExecute(String strResponce) {
		...
		LogConfig.logd("Parent unfollow responce =",""+strResponce);
		if(strResponce != null && strResponce.length() > 0) {
			...
			try {
			   ...
			   if (jObjectResult.getString("status").equalsIgnoreCase("success")) {
				   Common.arrLeaderBoardParentList.get(position).setFollow_status("0");
				   mParentUnfollowCallBack.requestParentUnfollowCallBack();
			   }
